class LobbyState extends EstadoBase {
  ...
  entrar() {
    ...
    const savedChar = localStorage.getItem('perfil.personagem');
    const savedWeapon = localStorage.getItem('perfil.arma');
    const savedBiome = localStorage.getItem('perfil.bioma');
    if (savedChar) {
    ...
    try {
      const savedMut = JSON.parse(localStorage.getItem('perfil.mutators') || '[]');
  ...
  aoPressionarTecla() {
    ...
    if (this.activePanel === 'CHAR') {
      if (keyCode === LEFT_ARROW) this.charIndex = (this.charIndex - 1 + this.characters.length) % this.characters.length;
        ...
        localStorage.setItem('perfil.personagem', chosen.id);
    ...
    if (this.activePanel === 'WEAPON') {
      ...
      if (keyCode === LEFT_ARROW) this.weaponIndex = (this.weaponIndex - 1 + uw.length) % uw.length;
        ...
        localStorage.setItem('perfil.arma', chosenW);
        this.activePanel = null;
      } else if (keyCode === ESCAPE) {
        this.activePanel = null;
      } else if (key === 'q' || key === 'Q') {
    ...
    if (this.activePanel === 'BIOME') {
      ...
      if (keyCode === LEFT_ARROW) this.biomeIndex = (this.biomeIndex - 1 + this.biomeKeys.length) % this.biomeKeys.length;
        ...
        localStorage.setItem('perfil.bioma', chosenB);
        this.activePanel = null;
      } else if (keyCode === ESCAPE) {
        this.activePanel = null;
      } else if (key === 'q' || key === 'Q') {
    ...
    if (this.activePanel === 'SIGILS') {
      ...
      if (keyCode === ENTER) {
        ...
        localStorage.setItem('perfil.mutators', JSON.stringify(enabled));
        this.activePanel = null;
      } else if (keyCode === ESCAPE) {
        this.activePanel = null;
      }
    ...
    if (this.activePanel === 'TRIAL') {
      if (keyCode === LEFT_ARROW) this.trialIndex = (this.trialIndex - 1 + this.trials.length) % this.trials.length;
        ...
        localStorage.setItem('perfil.trial', chosen.key);
    ...
    if (keyCode === ENTER && jogador) {
      ...
      if (dPortal < this.portal.r + 20) {
        ...
        try {
          const enabled = JSON.parse(localStorage.getItem('perfil.mutators') || '[]');
          window.currentRunConfig = window.currentRunConfig || {};
