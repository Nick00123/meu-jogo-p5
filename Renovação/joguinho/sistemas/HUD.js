...
function desenharHUD() {
  ...
  let podeEntrarNoPortal = false;  // Valor padrão
  let bgAlpha = 150;
  ...
  textAlign(LEFT, TOP);
  textSize(16);
  text(`Vida: ${jogador.vida}/${UpgradeSystem.getMaxHealth()}`, 10, 10);
  
  // Score and coins
  fill(255, 255, 0);
  text(`Pontuação: ${pontuacao}`, 10, 30);
  fill(100, 255, 100);
  text(`Moedas: ${UpgradeSystem.totalMoedasObtidas}`, 10, 50);
...
function desenharBossHUD() {
  ...
  // Barra de Vida
  const vidaPercentual = chefe.vida / chefe.vidaMaxima;
  ...
  textSize(12);
  textAlign(CENTER, CENTER);
  text(`${Math.ceil(chefe.vida)} / ${chefe.vidaMaxima}`, barraX + barraWidth / 2, barraY + barraHeight / 2);
  pop();
}